...
Запускается так: ```python3 main1.py <port>```

![photo](images/test1_res.png)
![photo](images/test1_head.png)
![photo](images/test2_res.png)
![photo](images/test2_head.png)
![photo](images/test3_res.png)
![photo](images/test3_head.png)
...
Всё сделано в файле main2.py. Запускается также ```python3 main2.py <port>```
...
Если файл не получается вывести в консоль (например, это картинка, как у меня), то файл сохраняется и об этом появляется сообщение

![photo](images/test_client.png)
...
Вводим в cmd:
```
python(3) main2.py <port> [concurrency_level]
```

По умолчанию concurrency_level = 5
...
Скорость потока (Пусть будет $\cal V_{потока}$): 128 Кбит/с -> $128*10^3$ бит/с

Размер 1 пакета: 56 байт (Пусть будет $\cal L$) -> $56*2^3$ бит

Скорость передачи данных: 1 Мбит/с (Пусть будет $\cal V_{передачи}$) -> $10^6$ бит/с

Задержка: 5 мс (Как на практике $\cal d_{распространения}$) -> $5*10^{-3}$ с

Решение:

По идее итоговый ответ считаем так:

$\cal V_{потока} * \cal L$ (собрали целый пакет) +  $\cal V_{передачи} * \cal L$ (отправили пакет) + $\cal d_{распространения} = 56*2^3*2^{-7}*2^{-3} + 56*2^3*10^{-6} + 5*10^{-3} = 3,5*10^{-3} + 0,448*10^{-3} + 5*10^{-3} = 8,948*10^{-3}$
