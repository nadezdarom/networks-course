...
3. Какой порядковый номер у SYN TCP-сегмента, который используется для установления
   ...
   - Порядковый номер: Sequence Number (raw): 2874150519
   - SYN-сегмент, т.к. в поле flags установлен флаг на SYN
...
4. Какой порядковый номер у SYNACK-сегмента, отправленного сервером gaia.cs.umass.edu
   ...
   - Порядковый номер: Sequence Number (raw): 1435310655
   - Acknowledgment Number: 1    (relative ack number), Acknowledgment number (raw): 2874150520
   - к sequence number из прошлого запроса прибавил 1
   - Установлены соответсвующие флаги: syn и acknowledgment
...
5. Какой порядковый номер у TCP-сегмента, содержащего команду POST протокола HTTP?
   ...
   - Sequence Number (raw): 2874150520
...
6. Рассмотрите TCP-сегмент, содержащий команду POST протокола HTTP, как первый TCP-сегмент 
   ...
   - 1, 634, 13774, 26914, 41514, 73634 (это всё relative-номера), я тут итерировалась по next sequence number
   - 30.0970462, 30.0970584, 31.079396, 31.079866, 31.189214, 31.190297
   - 31.079323, 31.079850, 31.189140, 31.190273, 31.301940
   - 1, 2 - [The RTT to ACK the segment was: 0.108861000 seconds]
   - 3 - [The RTT to ACK the segment was: 0.109266000 seconds]
   - 4 - [The RTT to ACK the segment was: 0.109744000 seconds]
   - 5 - [The RTT to ACK the segment was: 0.110407000 seconds]
   - 6 - [The RTT to ACK the segment was: 0.112726000 seconds]
...
7. Чему равна пропускная способность (количество байтов, передаваемых в единицу
   ...
   - Размер файла: 152321 б, ответ на первый пакет получен 31.079323, на последний 31.412188: $152321 / (31.412188 - 30.0970462) = 115820.97 байт/с$
   - посмотрела в http-запросе размер файла, посчитала разницу между отправленным первым tcp-пакетом и последним пришедшним tcp-ответом, в котором точно уже был весь файл
