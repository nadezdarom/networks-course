...
4. Рассмотрите соответствующий ping-пакет, полученный в ответ на предыдущий. 
   ...
   - Тип: 0 (Echo (ping) reply), кодовый номер: 0
   - Checksum, Identifier (BE, LE), Sequence Number (BE, LE), Data
   - Снова все по 2 байта
...
1. Рассмотрите ICMP-пакет с эхо-запросом на вашем скриншоте. Отличается ли он от ICMP-пакетов
   ...
   - TTL другой (1 вместо 128), на этот пакет ответа не пришло, в поле Data одни нули
...
2. Рассмотрите на вашем скриншоте ICMP-пакет с сообщением об ошибке. В нем больше полей,
   ...
   - Появилось поле Unused (тут просто нули) и информация по IPv4 и ICMP из предыдущего пакета
...
3. Рассмотрите три последних ICMP-пакета, полученных исходным хостом. Чем эти пакеты
   ...
   - во 1 типом, у всех 3 тип 0 (Echo (ping) reply), также тут нет полей unused, информации по предыдущим IPv4 и ICMP
   - отправленным пакетам хватило TTL (у 3 с конца отправленного с исходного хоста пакета TTL 12, вместо 1, как у первого), чтобы дойти до конца
...
4. Есть ли такой канал, задержка в котором существенно превышает среднее значение? Можете
   ...
   - Да, есть
   - Один конец канала в Санкт-Петербурге, второй - в Москве. Средняя задержка вышла больше, чем между Москвой и Вашингтоном. Могу предположить, что так вышло из-за того, что последние принадлежат CloudFlareNet
...
#### Демонстрация работы
Тут ip адрес отличается от тех, что был ранее, поскольку запуск был с ubuntu (почему) 
...
#### Демонстрация работы
По адресу 127.0.0.1:8000 я записутила свое приложение, поэтому свободные порты такие:
