...
3. Каковы значения Transaction-ID в наборе (Request/ACK) DHCP-сообщений? Опишите
   ...
   - Request: 0xe436a083, ACK: 0xe436a083. Transaction ID - уникальный id в 4 байта, который генерирует клиент, чтобы отличать сообщения между клиентом и dhcp-сервером (идентифицировать "свою" сессию)
...
4. Хост использует DHCP-протокол, в частности, для получения IP-адреса. Но IP-адрес хоста
   ...
   - исходный 0.0.0.0, конечный 255.255.255.255 (широковещательная рассылка)
...
6. Объясните назначение срока аренды IP-адреса. Какова длительность срока аренды в
   вашем эксперименте?
   - 1 день. Чтобы неактивные клиенты не занимали доступные IP-адреса, dhcp сервер дает их в аренду. 
